  ...
  if matches[1]:lower() == 'clean' and matches[2]:lower() == 'blacklist' then
    ...
    local function cleanbl(ext, res)
      if tonumber(res.total_count_) == 0 then -- «Blocklist is empty or maybe Bot is not group's admin»
        return tdcli.sendMessage(ext.chat_id, ext.msg_id, 0, '⚠️ لیست مسدودی های گروه خالی است', 1, 'md')
      end
      local x = 0
    ...
    return tdcli.getChannelMembers(msg.to.id, 0, 'Kicked', 900, cleanbl, {chat_id = msg.to.id, msg_id = msg.id}) -- «Gets channel blocklist»
  end
