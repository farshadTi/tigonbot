--Begin GroupManager.lua By TIGON
...
local function delmsg (TIGONBOT,TIGON)
    msgs = TIGONBOT.msgs 
    for k,v in pairs(TIGON.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(TIGON.messages_[0].chat_id_,{[0] = TIGON.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(TIGON.messages_[0].chat_id_, TIGON.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
...
if not lang then
 return "》*#lø¢κ* _ρøšт łš αlяeαđч lø¢κeđ_🔒\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "TIGON").."]"
elseif lang then
 return "》ارسال #لینک در گروه  از قبلا ممنوع است🔒\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..check_markdown(msg.from.username or "TIGON").."]"
end
else
